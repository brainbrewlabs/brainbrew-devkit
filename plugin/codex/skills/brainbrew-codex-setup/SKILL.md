...
3. If `brainbrew codex init` reports a missing runner, build or reinstall the package so `plugin/scripts/codex-runner.cjs` exists.
...
5. If MCP tools are needed, compare `codex mcp list` with the Codex server definition in `plugin/codex/mcp.json`.
